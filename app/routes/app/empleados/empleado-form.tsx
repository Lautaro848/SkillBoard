    ...
    <form ref={formRef} onSubmit={onSubmit} className="flex flex-col gap-6" noValidate>
      ...
      <section className={pestania === "personales" ? "flex flex-col gap-4" : "hidden"}>
        <div className="flex items-center gap-4">
          {preview ? (
            ...
            <div className="flex h-20 w-20 items-center justify-center rounded-full bg-[var(--color-border)] text-sm text-[var(--color-text-muted)]">
              Sin foto
            </div>
          )}
          <div>
            <label className="block text-sm font-medium" htmlFor="fotoInput">
              Foto
            </label>
            <input id="fotoInput" type="file" accept="image/jpeg,image/png,image/webp" onChange={onFotoChange} className="mt-1 text-sm" />
            {fotoError && <p className="mt-1 text-xs text-[var(--color-danger)]">{fotoError}</p>}
          </div>
        </div>
      ...
      <section className={pestania === "puesto" ? "flex flex-col gap-4" : "hidden"}>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select id="estado" name="estado" defaultValue={v.estado ?? "activo"} className="mt-1 block w-full rounded-md border border-[var(--color-border)] px-3 py-2 text-sm">
        ...
        <div>
          ...
          <textarea id="observaciones" name="observaciones" defaultValue={v.observaciones} maxLength={2000} rows={3} className="mt-1 block w-full rounded-md border border-[var(--color-border)] px-3 py-2 text-sm" />
          {errores.observaciones && <p className="mt-1 text-xs text-[var(--color-danger)]">{errores.observaciones[0]}</p>}
        </div>
      </section>
      ...
      <section className={pestania === "aptitudes" ? "flex flex-col gap-4" : "hidden"}>
        <div className="flex flex-wrap items-end gap-3">
          <div>
            <label className="text-sm font-medium" htmlFor="nuevaAptitud">
              Aptitud
            </label>
            <select id="nuevaAptitud" value={nuevaAptitud} onChange={(e) => setNuevaAptitud(e.target.value)} className="mt-1 block rounded-md border border-[var(--color-border)] px-3 py-1.5 text-sm">
          ...
          <div>
            <label className="text-sm font-medium" htmlFor="nuevoNivel">
              Nivel
            </label>
            <select id="nuevoNivel" value={nuevoNivel} onChange={(e) => setNuevoNivel(Number(e.target.value))} className="mt-1 block rounded-md border border-[var(--color-border)] px-3 py-1.5 text-sm">
              {[1, 2, 3, 4, 5].map((n) => (
          ...
          <button type="button" onClick={agregarAptitud} className="rounded-md border border-[var(--color-border)] px-3 py-1.5 text-sm font-medium">
            Agregar
          </button>
        </div>
        ...
        {aptitudes.length === 0 ? (
          ...
          <ul className="flex flex-col gap-2">
            {aptitudes.map((a) => {
              ...
              return (
                <li key={a.aptitudId} className="flex items-center justify-between rounded-md border border-[var(--color-border)] px-3 py-2 text-sm">
                  <span>
                    {nombre} — {NIVEL_ETIQUETAS[a.nivel]}
                  </span>
                  <button type="button" onClick={() => quitarAptitud(a.aptitudId)} className="text-[var(--color-text-muted)] underline">
                    Quitar
                  </button>
                </li>
              );
      ...
      <button type="submit" disabled={enviando} className="w-fit rounded-md bg-[var(--color-primary)] px-4 py-2 text-sm font-medium text-[var(--color-primary-contrast)] disabled:opacity-60">
...
function Selector({
  ...
  return (
    <div>
      ...
      <select id={name} name={name} required={required} defaultValue={defaultValue} className="mt-1 block w-full rounded-md border border-[var(--color-border)] px-3 py-2 text-sm">
        <option value="">Elegir...</option>
        {opciones.map((o) => (
